...
export default function HomepageStories(pinPosts) {
  ...
  // Get Guide Stories
  const { data: guidesData, error: guidesError } = useQuery(
    GetGuideHomepageStories,
    {
      fetchPolicy: 'network-only',
      nextFetchPolicy: 'cache-and-network',
    },
  ...
  useEffect(() => {
    const shuffleGuidePost = () => {
      ...
      guidesData?.contentNodes?.edges?.forEach((post) => {
        ...
        if (!uniqueDatabaseIds.has(databaseId)) {
          ...
          contentGuides.push(post.node) // Push the unique post to the array
        }
      })

      // Shuffle only the otherBannerAds array
      const shuffleGuidePost = shuffleArray(contentGuides)

      // Concatenate the arrays with pinned ads first and shuffled other banner ads
      const shuffledGuideArray = [...shuffleGuidePost]

      // Get the last two elements
      const lastTwoGuides = shuffledGuideArray.slice(-2)

      setGuideArray(lastTwoGuides)
    }
  ...
  const getGuidePost = [...GuideArray]
  const numberOfGuide = GuideArray.length
           ...
           {/* Honors Circle Stories */}
            ...
            {index - 1 === 2 && (
              <div className={cx('guide-wrapper')}>
                {/* Guide Stories */}
                {numberOfGuide !== 0 && numberOfGuide > 1 && (
                  <GuidePostTwoColumns
                    title={getGuidePost[1]?.title}
                    excerpt={getGuidePost[1]?.excerpt}
                    uri={getGuidePost[1]?.uri}
                    featuredImage={getGuidePost[1]?.featuredImage?.node}
                  />
                )}
              </div>
            )}
